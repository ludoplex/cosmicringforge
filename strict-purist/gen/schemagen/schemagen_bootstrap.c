/* MBSE Stacks — Schema Generator Bootstrap
 * Ring 0: Pure C, minimal bootstrap
 *
 * This is the STAGE 0 bootstrap - hand-written types.
 * Run this on specs/schemagen.schema to generate schemagen_types.h
 * Then schemagen.c includes generated types (self-hosting).
 *
 * Bootstrap sequence:
 *   1. cc schemagen_bootstrap.c -o schemagen-bootstrap
 *   2. ./schemagen-bootstrap ../specs/schemagen.schema .
 *   3. cc schemagen.c -o schemagen (includes generated schemagen_types.h)
 *
 * Usage: schemagen_bootstrap <input.schema> [output_dir]
 */
...
/* ── Bootstrap Type System (Hand-Written) ───────────────────────── */
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "schemagen-bootstrap %s (Stage 0 Bootstrap)\n", SCHEMAGEN_VERSION);
        fprintf(stderr, "Usage: schemagen_bootstrap <input.schema> [output_dir] [prefix]\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "Bootstrap sequence:\n");
        fprintf(stderr, "  1. cc schemagen_bootstrap.c -o schemagen-bootstrap\n");
        fprintf(stderr, "  2. ./schemagen-bootstrap specs/schemagen.schema gen/schemagen SCHEMAGEN\n");
        fprintf(stderr, "  3. cc schemagen.c -o schemagen (uses generated types)\n");
        return 1;
    }
    ...
    fprintf(stderr, "[bootstrap] Parsed %d types from %s\n", type_count, input);
    ...
    fclose(out);
    fprintf(stderr, "[bootstrap] Generated %s\n", path);
    ...
    fprintf(stderr, "[bootstrap] Bootstrap complete\n");
    return 0;
}
